...
export const Navigation = () => {
  ...
  return (
    ...
    >
      <div
        className="container mx-auto rounded-xl border px-4 sm:px-6 py-3"
        style={{
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span className="font-heading text-lg">
              Mehedy<span className="text-muted-foreground">.dev</span>
            </span>
          </motion.div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden lg:flex items-center gap-2 mr-2 border-r pr-3" style={{ borderColor: 'hsl(var(--border))' }}>
              ...
              <a href="https://codeforces.com/profile/mehedyk" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors" title="CodeForces">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M4.5 7.5C5.328 7.5 6 8.172 6 9v10.5c0 .828-.672 1.5-1.5 1.5h-3C.672 21 0 20.328 0 19.5V9c0-.828.672-1.5 1.5-1.5h3zm9-4.5c.828 0 1.5.672 1.5 1.5v15c0 .828-.672 1.5-1.5 1.5h-3c-.828 0-1.5-.672-1.5-1.5v-15c0-.828.672-1.5 1.5-1.5h3zm9 7.5c.828 0 1.5.672 1.5 1.5v7.5c0 .828-.672 1.5-1.5 1.5h-3c-.828 0-1.5-.672-1.5-1.5V12c0-.828.672-1.5 1.5-1.5h3z"/></svg>
              </a>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="xl:hidden pt-4 pb-2"
            >
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className="block w-full text-left py-2 text-sm hover:text-primary transition-colors font-mono"
                >
                  {t(item.labelKey)}
                </button>
              ))}
              ...
              {/* Mobile Social Links */}
              <div className="flex items-center gap-3 pt-4 mt-2 border-t" style={{ borderColor: 'hsl(var(--border))' }}>
                <a href="https://www.facebook.com/mahdi.kawser" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>
                <a href="https://github.com/mehedyk" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors"><Github className="w-5 h-5" /></a>
                <a href="https://www.linkedin.com/in/mehedyk/" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors"><Linkedin className="w-5 h-5" /></a>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
